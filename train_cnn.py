...
def make_env(seed=0):
    def _init():
        env = TerisEnv(seed=seed)
        env = ActionMasker(env, TerisEnv.get_action_mask)
        env = Monitor(env)
        env.seed(seed)
        return env
    return _init
...
def main():
    ...
    checkpoint_callback = CheckpointCallback(save_freq=checkpoint_interval, save_path=save_dir, name_prefix="ppo_teris")
    ...
    model.save(os.path.join(save_dir, "ppo_teris_final.zip"))
